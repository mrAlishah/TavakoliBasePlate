...
var ErrCount = &Metric{
	ID:          "ErrCount",
	Name:        "ErrCount",
	Description: "number of errors occurred by this type",
	Type:        "counter_vec",
	Args:        []string{"type"},
}
...
var OkCount = &Metric{
	ID:          "OkCount",
	Name:        "OkCount",
	Description: "number of successful call bu this type",
	Type:        "counter_vec",
	Args:        []string{"type"},
}

var queueStatus = &Metric{
	ID:          "queueStatus",
	Name:        "queueStatus",
	Description: "monitoring queue",
	Type:        "histogram_vec",
	Args:        []string{"type"},
}
var respTime = &Metric{
...
var ingestRespTime = &Metric{
	ID:          "ingestRespTime",
	Name:        "ingestRespTime",
	Description: "ingest response time",
	Type:        "histogram_vec",
	Args:        []string{"type"},
}
var cacheHit = &Metric{
	ID:          "cache_hit",
	Name:        "cache_hit",
	Description: "how many requests handled by cache",
	Type:        "counter_vec",
	Args:        []string{"type"},
}
...
func (p *Prometheus) IncError(label string) {
	p.ErrCount.WithLabelValues(label).Inc()
}

func (p *Prometheus) IncCalls(label string) {
	p.OkCount.WithLabelValues(label).Inc()
}

func (p *Prometheus) SetRespTime(label string, value float64) {
	p.respTime.WithLabelValues(label).Observe(value)
}

func (p *Prometheus) IncCacheCounter(label string) {
	p.cacheHit.WithLabelValues(label).Inc()
}

//func  (p *Prometheus)  SummaryByType(event string, num float64, mu string) {
//	p.summaryByType.WithLabelValues(event, mu).Observe(num)
//}
//
...
func (p *Prometheus) registerMetrics(subsystem string) {
	...
	for _, metricDef := range p.MetricsList {
		...
		if err := prometheus.Register(metric); err != nil {
			logrus.WithError(err).Errorf("%s could not be registered in Prometheus", metricDef.Name)
		}
		switch metricDef {
		case reqCnt:
		...
		case ErrCount:
			p.ErrCount = metric.(*prometheus.CounterVec)
		case OkCount:
			p.OkCount = metric.(*prometheus.CounterVec)
		case cacheHit:
			p.cacheHit = metric.(*prometheus.CounterVec)
		case respTime:
			p.respTime = metric.(*prometheus.HistogramVec)
		}
